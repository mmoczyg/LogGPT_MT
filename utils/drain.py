...
class LogParser:
    ...
    def treeSearch(self, rn, seq):
        ...
        if retLogClust is not None and not isinstance(retLogClust, Logcluster):
            raise TypeError("treeSearch returned non-Logcluster instances")
    ...
    def fastMatch(self, logClustL, seq):
        ...
        for logClust in logClustL:
            if not isinstance(logClust, Logcluster):
                continue
    ...
    def json_to_dataframe(self, log_file):  # 😀️
        ...
        logdf.insert(0, 'LineId', None)
        logdf['LineId'] = [i + 1 for i in range(len(logdf))]
        return logdf
    ...
    def get_parameter_list(self, row):
        ...
        if "<*>" not in template_regex:
            return []
        template_regex = re.sub(r'([^\w\s])', r'\\\1', template_regex)
        template_regex = re.sub(r'\\ +', r'\s+', template_regex)
        template_regex = "^" + \
            template_regex.replace("\\<\\*\\>", "(.*?)") + "$"
        parameter_list = re.findall(template_regex, row["msg"])
